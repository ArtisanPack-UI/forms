<div>
    @if ($this->submission)
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <x-artisanpack-button
                            ...
                            :label="$this->submission->is_starred ? __( 'Starred' ) : __( 'Star' )"
                        ...
                        <x-artisanpack-button
                            ...
                            :label="$this->submission->is_spam ? __( 'Spam' ) : __( 'Mark Spam' )"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <x-artisanpack-card :title="__( 'Submission Data' )" separator>
                            ...
                            <div class="py-4 first:pt-0 last:pb-0">
                                ...
                                <dd class="mt-1 text-sm">
                                    ...
                                    @elseif ($value->value !== null && $value->value !== '')
                                        ...
                                        @endif
                                    @else
                                        <span class="italic opacity-40">{{ __( 'No response' ) }}</span>
                                    @endif
                                </dd>
                            </div>
                        @empty
                            <div class="py-8 text-center opacity-60">{{ __( 'No data submitted.' ) }}</div>
                        @endforelse
                    </div>
                </x-artisanpack-card>

                <!-- Admin Notes -->
                <x-artisanpack-card :title="__( 'Admin Notes' )" separator>
                    <x-slot:subtitle>{{ __( 'Internal notes about this submission (auto-saved)' ) }}</x-slot:subtitle>
                    <x-artisanpack-textarea
                        wire:model.live.debounce.500ms="adminNotes"
                        rows="4"
                        :placeholder="__( 'Add notes about this submission...' )"
                    />
                    <p class="mt-2 text-xs opacity-60">
                        <span wire:loading.remove wire:target="adminNotes">{{ __( 'Notes are saved automatically' ) }}</span>
                        <span wire:loading wire:target="adminNotes">{{ __( 'Saving...' ) }}</span>
                    </p>
                </x-artisanpack-card>

                <!-- Attachments -->
                @if ($this->submission->uploads->isNotEmpty())
                    <x-artisanpack-card :title="__( 'Attachments' )" separator>
                        <div class="space-y-3">
            ...
            <div class="space-y-6">
                <!-- Metadata Card -->
                <x-artisanpack-card :title="__( 'Details' )" separator>
                    <dl class="space-y-4">
                        <!-- Form Link -->
                        <div>
                            <dt class="text-sm font-medium opacity-60">{{ __( 'Form' ) }}</dt>
                            <dd class="mt-1 text-sm">
                        ...
                        <!-- Submission Number -->
                        <div>
                            <dt class="text-sm font-medium opacity-60">{{ __( 'Submission Number' ) }}</dt>
                            <dd class="mt-1 text-sm font-mono">{{ $this->submission->submission_number }}</dd>
                        </div>

                        <!-- Submitted Date/Time -->
                        <div>
                            <dt class="text-sm font-medium opacity-60">{{ __( 'Submitted At' ) }}</dt>
                            <dd class="mt-1 text-sm">
                        ...
                        @if ($this->submission->page_url)
                            <div>
                                <dt class="text-sm font-medium opacity-60">{{ __( 'Page URL' ) }}</dt>
                        ...
                        @if ($this->submission->referrer_url)
                            <div>
                                <dt class="text-sm font-medium opacity-60">{{ __( 'Referrer' ) }}</dt>
                                <dd class="mt-1 text-sm break-all">
                        ...
                        @if ($this->submission->ip_address)
                            <div>
                                <dt class="text-sm font-medium opacity-60">{{ __( 'IP Address' ) }}</dt>
                        ...
                        @if ($this->submission->user_agent)
                            <div>
                                <dt class="text-sm font-medium opacity-60">{{ __( 'User Agent' ) }}</dt>
                ...
                <x-artisanpack-card separator>
                    <div class="space-y-2">
                        ...
                        <x-artisanpack-button
                            ...
                            :label="$this->submission->is_read ? __( 'Mark Unread' ) : __( 'Mark Read' )"
                            color="ghost"
                        ...
                        <x-artisanpack-button
                            wire:click="toggleStar"
                            :icon="$this->submission->is_starred ? 's-star' : 'o-star'"
                            :label="$this->submission->is_starred ? __( 'Unstar' ) : __( 'Star' )"
                            :color="$this->submission->is_starred ? 'warning' : 'ghost'"
                            class="w-full justify-start"
                        />
                        ...
                        <x-artisanpack-button
                            ...
                            :label="$this->submission->is_spam ? __( 'Not Spam' ) : __( 'Mark as Spam' )"
                            :color="$this->submission->is_spam ? 'error' : 'ghost'"
                            class="w-full justify-start"
                        />
                    </div>
    ...
    @else
        ...
        <x-artisanpack-card class="p-12 text-center">
            ...
            <h3 class="mt-2 text-sm font-medium">{{ __( 'Submission not found' ) }}</h3>
            <p class="mt-1 text-sm opacity-60">
                {{ __( "The submission you're looking for doesn't exist or has been deleted." ) }}
            </p>
        </x-artisanpack-card>
    @endif
</div>
